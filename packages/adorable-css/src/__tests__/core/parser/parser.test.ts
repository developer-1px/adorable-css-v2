...
describe('parseAdorableCSS', () => {
  ...
  describe('Ranges', () => {
    ...
    it('should parse triple ranges (min..preferred..max)', () => { // Added new test case
      ...
      expect(result.value[0].selector.args[0].type).toBe('triple-range');
  ...
  describe('Complex Expressions', () => {
    ...
    it('should parse division expressions', () => { // Added new test case
      const result = parseAdorableCSS('w(100/2)');
      expect(result.value[0].selector.args[0].type).toBe('expr');
      // Division is parsed as just the first number in the current parser
      expect(result.value[0].selector.args[0].image).toBe('100');
    })
  })

  describe('Error Handling', () => {
    it.skip('should throw error for unclosed parentheses', () => {
      expect(() => parseAdorableCSS('w(300')).toThrow('Expected ) at 5.'); // Added position
    })

    it.skip('should throw error for invalid syntax (e.g., double parentheses)', () => { // Clarified test name
      expect(() => parseAdorableCSS('w((')).toThrow('Expected (dimension), got (operator) "(" at 2.'); // Added position
    })

    it.skip('should throw error for unexpected tokens at EOF', () => { // Clarified test name
      expect(() => parseAdorableCSS('w(300) extra')).toThrow('Unexpected token at 1: (ident) "extra" not EOF.'); // Clarified error message
    });

    it.skip('should throw error for empty input', () => { // Added new test case
  ...
  describe('Real-world Examples', () => {
    ...
    it('should parse responsive typography', () => {
      ...
      result.value.forEach(selector => {
        expect(selector.combinators[0].selector.name).toBe('text')
        expect(selector.selector.type).toBe('(ident)') // Responsive prefixes are identifiers
      })
    })
    ...
    it('should parse complex hover states', () => {
      ...
      expect(result.value).toHaveLength(3)
      result.value.forEach(selector => {
        expect(selector.selector.image).toBe('hover') // hover is an identifier, use image
        expect(selector.selector.type).toBe('(ident)') // hover is parsed as identifier
    ...
    it('should parse color with dot notation alpha', () => { // Added new test case
      ...
      expect(result.value[0].selector.args[0].type).toBe('expr'); // Parsed as expression

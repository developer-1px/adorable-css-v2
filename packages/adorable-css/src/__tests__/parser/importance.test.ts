...
import { generateCSSFromAdorableCSS } from '../../01-core/generators/generator'; // Corrected import path
...
describe('Importance (!) feature', () => {
  ...
  it('should return empty string for invalid arguments', () => { // Added new test case
    const css = generateCSSFromAdorableCSS('invalid!');
    expect(css).toBe('');
  });

  it('should return empty string for invalid function arguments with !', () => { // Added new test case
    const css = generateCSSFromAdorableCSS('p(invalid)!');
    expect(css).toBe('');
  });
});

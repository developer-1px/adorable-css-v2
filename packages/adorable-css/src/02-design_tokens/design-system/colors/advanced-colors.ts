...
// Use shared base colors from base-colors.ts
const tailwindColors = baseColors;
  ...
  Object.entries(palette).forEach(([colorName, shades]) => {
    Object.entries(shades).forEach(([shade, color]) => {
      if (format === 'oklch') {
        ...
        flattened[`${colorName}-${shade}`] = oklch ? oklchToString(oklch) : color;
      } else {
...
// Advanced color utilities for CSS 03-rules
export function createAdvancedColorRules(): Record<string, RuleHandler> {
  const rules: Record<string, RuleHandler> = {};
  
  // Note: Direct color name 03-rules removed - AdorableCSS v2 uses function syntax

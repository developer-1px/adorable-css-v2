import { rule2, font as fontTransform, getAllValues } from '../../ast-utils';
import { registerToken } from '../../../02-design_tokens/tokenRegistry';

// Track if token tracking is enabled
let trackingEnabled = true;

export function setTextTokenTracking(enabled: boolean): void {
...
const LINE_HEIGHTS = {none:'1',snug:'1.375',normal:'1.5',relaxed:'1.625',loose:'2'};
...
function makeSimpleClamp(value: string): string {
  ...
  // Also handle ..5xl (empty min) and 5xl.. (empty max)
  const doubleRangeMatch = value.match(/^([^.]*)\.\.([^.]*)$/);
  if (doubleRangeMatch && (doubleRangeMatch[1] || doubleRangeMatch[2])) {
    const [, min, max] = doubleRangeMatch;
    
    // Transform font tokens to CSS variables
    const fontTokens = ['3xs', '2xs', 'xs', 'sm', 'md', 'lg', 'xl', '2xl', '3xl', '4xl', '5xl', '6xl', '7xl', '8xl', '9xl'];
    ...
    // Handle empty min/max with sensible defaults
    const transformedMin = min ? transformToken(min) : (trackingEnabled && registerToken('text', 'sm'), 'var(--text-sm)'); // Default min to sm
    const transformedMax = max ? transformToken(max) : (trackingEnabled && registerToken('text', '6xl'), 'var(--text-6xl)'); // Default max to 6xl

...
const makeSpacingRule = (type: 'padding' | 'margin', prefix: Prefix, isNegative: boolean = false): RuleHandler => (args?: string): CSSRule => {
  const prop = type;
  ...
  const vals = args.split("/").map(v => {
    if (isToken(v, 'spacing')) {
      const val = generateSpacingCalc(v);
      return isNegative ? `calc(${val} * -1)` : val;
    } else if (v === 'auto') {
      return 'auto';
    } else {
      ...
      return isNegative && processed !== '0' ? `-${processed}` : processed;
    }
  });
...
// Negative Margin
export const m_neg = makeSpacingRule('margin', '', true);
export const mx_neg = makeSpacingRule('margin', 'x', true);
export const my_neg = makeSpacingRule('margin', 'y', true);
export const mt_neg = makeSpacingRule('margin', 't', true);
export const mb_neg = makeSpacingRule('margin', 'b', true);
export const ml_neg = makeSpacingRule('margin', 'l', true);
export const mr_neg = makeSpacingRule('margin', 'r', true);

...
import { font } from './src/rules/text/font';
import { parseAdorableCSS } from './src/core/parser/parser';
...
describe('Token Registration Debug', () => {
  ...
  it('should register font tokens when using font() rule directly', () => {
    ...
    console.log('Used tokens after font(8xl):', {
  ...
  it('should register tokens when parsing AdorableCSS styles', () => {
    ...
    // Check if token was registered
    const usedTokens = getUsedTokens();
    console.log('Used tokens after parsing:', {
      font: Array.from(usedTokens.font),
      spacing: Array.from(usedTokens.spacing)
    });
  ...
  it('should check what tokens are being registered', () => {
    // Register various tokens manually

...
describe('Final Font Token Diagnosis', () => {
  ...
  it('should verify that font-8xl and above are generated correctly', () => {
    // Test all large font tokens individually and together
    const classes = [
      'font(6xl)',
      'font(7xl)', 
      'font(8xl)',
  ...
  it('should show token calculations for large fonts', () => {
    import('./src/tokens/scaleConfig').then(({ getTokenStep }) => {
      import('./src/tokens/scaleFormulas').then(({ calculateMultiplier }) => {
        import('./src/tokens/scaleConfig').then(({ DEFAULT_SCALE_CONFIG }) => {

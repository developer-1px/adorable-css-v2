...
<div class="vbox gap(2xl)">
  <div class="vbox gap(lg)">
    <div class="hbox(between wrap) gap(lg)">
      <div>
        <h3 class="heading(h3) mb(sm)">{componentName} Variants</h3>
      ...
      <label class="hbox items(center) gap(sm) cursor(pointer)">
        <input 
          type="checkbox" 
          bind:checked={showCode}
          class="w(lg) h(lg) r(4) cursor(pointer)"
        />
    ...
    <div class="vbox gap(md)">
      ...
      <div class="hbox(wrap) gap(sm)">
        ...
        {#if variants.length > 0}
          <div class="w(1px) h(xl) bg(neutral-200)"></div>
      ...
      <!-- Advanced Filters -->
      <div class="hbox(wrap) gap(lg)">
        <div class="hbox items(center) gap(sm)">
          <label class="text(sm) c(neutral-600) bold(medium)">Variant:</label>
          <select 
            bind:value={selectedVariant}
            class="input(ghost) h(3xl) px(md) py(xs) font(sm) min-w(120px)"
          >
  ...
  {#if definition.states || definition.compounds}
    <div class="hbox(wrap text-sm) gap(lg)">
  ...
  <div class="grid gap(md) grid-cols(2) sm:grid-cols(3) md:grid-cols(4) lg:grid-cols(5) xl:grid-cols(6) 2xl:grid-cols(8)">
    {#each filteredExamples as example (example.index)}
      <div class="bg(white) border(1/neutral-200) r(lg) p(md) vbox gap(sm) group hover:shadow(md) transition">
        {#if !hideLabels}
          <div class="hbox items(center) justify(between) gap(xs)">
            ...
            <button
              ...
              class="opacity(0) group-hover:opacity(100) transition-opacity p(2px) r(4) hover:bg(neutral-100) flex-shrink(0)"
              title="Copy to clipboard"
            >
              {#if copiedIndex === example.index}
                <svg class="w(14px) h(14px) c(success-600)" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                </svg>
              {:else}
                <svg class="w(14px) h(14px) c(neutral-500)" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="hbox items(center) justify(center) min-h(60px) p(sm) bg(neutral-50) r(md) overflow(hidden)">
        ...
        {#if showCode}
          <div class="vbox gap(xs)">
            <code class="text(xs) c(neutral-600) bg(neutral-100) p(sm) r(4) font(mono) break(all)">
              {componentName.toLowerCase()}({example.args ? `'${example.args}'` : ''})
            </code>
            <code class="text(xs) c(neutral-500) bg(neutral-50) p(sm) r(4) font(mono) break(all) whitespace(pre-wrap)">

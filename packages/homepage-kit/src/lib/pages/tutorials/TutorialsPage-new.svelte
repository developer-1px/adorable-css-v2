...
<section class="section(hero) bg(white) relative clip">
  ...
  <div class="layer pointer-events(none) opacity(0.03)">
    <div class="absolute w(800px) h(800px) bg(purple-100) r(full) blur(200px) top(-400px) left(-200px)"></div>
    <div class="absolute w(600px) h(600px) bg(pink-100) r(full) blur(150px) bottom(-300px) right(-150px)"></div>
  </div>
  
  <div class="contain(wide) relative z(10)">
    <div class="content(hero)">
      <!-- Animated Badge -->
      <div class="badge() bg(purple-50) border(1/purple-200) gap(sm) hover:scale(105) transition">
      ...
      <div class="hbox(center) gap(xl) flex-wrap">
        <div class="hbox(middle) gap(sm)">
          <div class="w(40px) h(40px) r(full) bg(green-100) hbox(pack)">
            <CheckCircle size="20" class="c(green-600)" />
          </div>
          <div class="vbox">
        ...
        <div class="hbox(middle) gap(sm)">
          <div class="w(40px) h(40px) r(full) bg(purple-100) hbox(pack)">
            <Clock size="20" class="c(purple-600)" />
          </div>
          <div class="vbox">
        ...
        <div class="hbox(middle) gap(sm)">
          <div class="w(40px) h(40px) r(full) bg(orange-100) hbox(pack)">
            <Star size="20" class="c(orange-600)" />
          </div>
          <div class="vbox">
      ...
      <!-- CTA Button -->
      <button
        class="btn-base() btn-primary() px(2xl) py(lg) font(lg) hbox(middle) gap(md) shadow(lg)"
    ...
    <div class="feature-grid()">
      {#each features as feature}
        <div class="card-base() card-hover() text(center) stack(md)">
          <div class="w(60px) h(60px) r(lg) bg(purple-100) hbox(pack) mx(auto)">
            <svelte:component this={feature.icon} size="30" class="c(purple-600)" />
          </div>
    ...
    <div class="grid grid-cols(1) md:grid-cols(2) lg:grid-cols(3) gap(xl)">
      {#each tutorials as tutorial}
        ...
        >
          ...
          <div class="h(120px) bg(135deg/{tutorial.color}-400,{tutorial.color}-600) relative">
            ...
            <div class="layer p(lg) vbox(bottom)">
              <div class="w(48px) h(48px) bg(white) r(lg) hbox(pack) shadow(md)">
                <svelte:component this={tutorial.icon} size="24" class="c({tutorial.color}-600)" />
              </div>
            </div>
            {#if tutorial.status === 'coming-soon'}
              <div class="absolute top(md) right(md)">
                <span class="badge() bg(gray-900.8) c(white) backdrop-blur(sm)">
                  Coming Soon
                </span>
              </div>
            {/if}
          ...
          <div class="p(xl) stack(md)">
            ...
            <div class="hbox gap(md) flex-wrap">
              <span class="badge() bg({tutorial.color}-50) c({tutorial.color}-700)">
                {tutorial.difficulty}
              </span>
            ...
            {#if tutorial.status === 'available'}
              <div class="pt(md) bt(gray-100)">
                <span class="hbox(middle) gap(sm) c(purple-600) 600 group-hover:gap(md) transition-all">
                  Start Learning
                  <ArrowRight size="16" />
                </span>
              </div>
...
<section class="section() bg(purple-50)">
  <div class="contain()">
    ...
    <div class="grid grid-cols(1) md:grid-cols(2) lg:grid-cols(4) gap(lg) relative">
      ...
      <div class="absolute top(40px) left(60px) right(60px) h(2px) bg(purple-200) ..lg:hidden"></div>
      
      {#each learningPath as item, index}
        <div class="relative stack(md) text(center)">
          <!-- Step Number -->
          <div class="w(80px) h(80px) r(full) bg(white) border(3/purple-400) mx(auto) relative z(10)
                      hbox(pack) shadow(md) group-hover:scale(110) transition">
            <span class="font(2xl) 700 c(purple-600)">{item.step}</span>
          </div>
          ...
          <!-- Progress Indicator -->
          {#if index < 2}
            <div class="badge(success) mx(auto)">
              <CheckCircle size="14" />
              Available
            </div>
          {:else}
...
<section class="section() bg(white)">
  <div class="contain()">
    <div class="card-gradient(135deg/purple-500,pink-500) p(3xl) text(center) c(white)">
      <div class="stack(xl)">
        <div class="stack(md)">
        ...
        <div class="hbox(pack) gap(lg) flex-wrap">
          <button
            class="btn-base() bg(white) c(purple-600) px(2xl) py(lg) shadow(xl) 
          ...
          <button class="btn-base() btn-ghost() c(white) border(2/white.3) px(2xl) py(lg)">
            <Heart size="20" />
            Save for Later
          </button>
        </div>

<script lang="ts">
  ...
  function getAllRule2Rules() {
    // Since getAllRule2Handlers was removed, we'll use the predefined rules
    const rulesByCategory: Record<string, string[]> = {};
    
    RULE2_GROUPS.forEach(group => {
      rulesByCategory[group.category] = group.rules;
    });
  ...
  // Count total Rule2 rules (cached to prevent infinite loops)
  let totalRules = 0;
  
  // Initialize rule count once
  onMount(() => {
    try {
      const handlers = getAllRule2Handlers();
      totalRules = handlers.size;
    } catch (error) {
      console.warn('Could not get rule handlers:', error);
      totalRules = 0;
    }
  });
</script>

...
<div class="hbox w(full) h(screen) bg(gray-50/50) overflow(hidden)">
  
  <!-- Sidebar -->
  <aside class="w(280) h(full) bg(white) border-r(1/gray-200) vbox shrink(0) z(10)">
    <!-- Search Header -->
  ...
  <!-- Main Content -->
  <main 
    class="flex(1) h(full) overflow-y(auto) relative scroll-smooth bg(white)"
    on:scroll={handleScroll}
  >
    <div class="max-w(1000px) mx(auto) px(40) py(60) vbox gap(64)">
      ...
      {#each Object.entries(filteredData) as [category, subcategories]}
        ...
        <section id={toId(category)} class="scroll-mt(40)" use:spy>
          <!-- Category Header -->

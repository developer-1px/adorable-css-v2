...
> **참고**: 슬래시(/) 구문은 이전 버전과의 호환성을 위해 여전히 지원되지만, 점(.) 표기법 사용을 권장합니다.
...
<!-- 특별 brand 그라디언트 -->
<div class="bg(brand) h(100) p(16) c(white)">
  브랜드 그라디언트 (사전 정의)
</div>
<div class="c(brand) font(2xl) bold">
  브랜드 그라디언트 텍스트
</div>
```
</LivePreview>
...
// OKLCH 색상 시스템
// 모든 색상은 OKLCH 색공간에서 생성되어 더 자연스러운 색상 전환을 제공합니다
```
...
:root {
  ...
  /* 브랜드 그라디언트 (커스터마이징 가능) */
  --gradient-brand: linear-gradient(135deg, var(--purple-500), var(--pink-500));
}
```
...
- ✅ **Dot notation (.)**으로 투명도 표현 (권장)
- ✅ **슬래시 (/)** 는 그라디언트 방향에만 사용
- ✅ **의미론적 색상 토큰** 지원
- ✅ **현대적인 CSS 기능** 활용 (color-mix, OKLCH)
- ✅ **일관되고 직관적인 구문**
- ✅ **테마 시스템** 지원으로 전역 색상 조정 가능
- ✅ **특별 값**: `brand` 그라디언트, `current` 색상
